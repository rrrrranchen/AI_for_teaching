...
class 同伴互动(Action):
    """学生之间基于画像的实时讨论"""
    PROMPT_TEMPLATE: str = """
    **角色**：学生{student_id}（{learning_style}学习者）
    **同伴消息**：{peer_message}
    **当前话题**：{topic}
    **我的知识画像**：
      强项：{strengths}
    ...
    **任务**：
    1. 用日常口吻回应同伴（可补充例子、提出质疑、给出提示）
    2. 若同伴的回答解决了你的弱项，更新自我理解程度
    3. 继续抛出一个推进讨论的新问题
    4. 用 JSON 返回：
    {{
        "reply": "给同伴的回复",
        "new_understanding": 4,   // 1-5 自我评估
        "follow_up": "继续讨论的问题"
    }}
    """
    ...
    async def run(self, student_profile: dict, peer_message: str, topic: str):
        prompt = self.PROMPT_TEMPLATE.format(
            student_id=student_profile["student_id"],
            learning_style=student_profile["learning_style"],
            strengths=", ".join(student_profile["strengths"]),
            weaknesses=", ".join(student_profile["weaknesses"]),
            learning_state=student_profile.get("learning_state", "讨论中"),
...
class 小组协作调度(Action):
    """教师触发小组讨论并把同伴消息路由到对应学生"""
    name: str = "小组协作调度"

    async def run(self, group: List[str], topic: str, starter_message: str):
        # 把 starter_message 发给组内所有学生
        for sid in group:
            student_agent = ...  # 通过 env 或 Role 容器找到对应学生智能体实例
            student_agent.peer_inbox.append({
                "from": "teacher",
