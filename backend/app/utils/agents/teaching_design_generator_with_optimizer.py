...
from app.utils.agents.evidence import EnhancedKnowledgeValidator, KnowledgeSupportValidator
from app.utils.agents.action import 优化教学设计, 学生画像构建器, 教学流程执行, 教学评估设计, 最终优化设计, 编写教学材料, 设计课程大纲
from app.utils.agents.role import 增强优化专家, 学生智能体, 教学材料编写者, 教学评估专家, 课堂分析师, 课堂模拟教师, 课程设计师
...
[
  ...
  },
  {
    ...
    """,
    knowledge_base: str = "根据教师输入从知识库中检索的参考资料",
    ...
    kb_client = KnowledgeSupportValidator()              
    validator  = EnhancedKnowledgeValidator(kb_client)

    for role in team.env.roles.values():
        for msg in role.rc.memory.get():
            validator.process_message(msg)

    trust_report = validator.get_trust_report(min_score=0.7)
    logger.info(f"系统可信度报告：{json.dumps(trust_report, ensure_ascii=False, indent=2)}")
    # 返回最终优化结果
