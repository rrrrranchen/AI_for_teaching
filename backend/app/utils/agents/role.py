...
import random
import re
from app.utils.agents.evidence import EvidenceChainValidator
import fire
...
from action import 上课模拟, 优化教学设计, 分析课堂报告, 同伴互动, 小组协作调度, 教学优化建议, 教学流程执行, 教学评估设计, 最终优化设计,   编写教学材料, 设计课程大纲, 课堂互动,  课堂观察评估
...
class 增强优化专家(Role):
    ...
    def __init__(self, **kwargs):
        ...
        # 初始化证据链验证器
        self.validator = EvidenceChainValidator(knowledge_base=self.rc.kb)
        self.max_reconstructions = 3  # 最大重构次数
    ...
    async def _validate_and_reconstruct(self, design: str) -> str:
        """验证设计并执行证据链重构"""
        # 1. 提取设计中的核心声明
        claims = self._extract_claims_from_design(design)
        
        # 2. 验证循环（最多重构3次）
        reconstruction_count = 0
        while reconstruction_count < self.max_reconstructions:
            validation_report = self.validator.validate_design(claims)
            
            # 检查是否需要重构
            if validation_report['overall_score'] >= 0.7:
                logger.info(f"验证通过: 可信度{validation_report['overall_score']:.2f}")
                return self._format_design_with_sources(claims)
            
            # 执行重构
            reconstruction_count += 1
            logger.warning(f"证据链重构 #{reconstruction_count} (可信度{validation_report['overall_score']:.2f})")
            
            for i, claim in enumerate(claims):
                if validation_report['claim_scores'][i] < 0.7:
                    # 重构低可信度声明
                    reconstruction = self.validator.reconstruct_evidence_chain(
                        claim['statement'],
                        context=self.rc.env.context
                    )
                    claims[i] = {
                        'statement': claim['statement'],
                        'evidence': reconstruction['new_evidence'],
                        'sources': self._format_sources(reconstruction['new_evidence'])
                    }
        
        # 3. 最终验证
        final_report = self.validator.validate_design(claims)
        if final_report['overall_score'] < 0.7:
            logger.error(f"最终验证失败: 可信度{final_report['overall_score']:.2f}")
            return "⚠️ 优化失败：无法达到最低可信度要求"
        
        return self._format_design_with_sources(claims)

    def _extract_claims_from_design(self, design: str) -> list:
        """从教学设计中提取声明和证据链（完整实现）"""
        claims = []
        ...
        for pattern in claim_patterns:
            ...
            extracted_statements.extend(matches)
        

        if not extracted_statements:

            paragraphs = [p.strip() for p in design.split('\n') if p.strip()]
            extracted_statements = paragraphs[:5] 
        

        for statement in extracted_statements:
            try:

                evidence_candidates = self.rc.kb.query(
                    query=statement,
                    top_k=5,
                    min_similarity=0.6,
                    filters={
                        "doc_type": ["教育理论", "研究报告", "教学指南"]
                    }
                )
                ...
                for candidate in evidence_candidates:
                    evidence_chain.append({
                        ...
                        "start": candidate.start_pos,
                        "end": candidate.end_pos,
                        "similarity": candidate.similarity
                    })
                

                sources = self._format_sources(evidence_chain)
                ...
                logger.error(f"提取声明失败: {statement} - {str(e)}")
    ...
    def _format_sources(self, evidence_list: list) -> str:
        """格式化来源标注"""
        sources = []
        for evidence in evidence_list:
            source = f"[来源：《{evidence['doc_id']}》{evidence.get('section', '')}]"
            if source not in sources:  
                sources.append(source)
        return " ".join(sources)

    def _format_design_with_sources(self, claims: list) -> str:
        """将声明重新组合为教学设计文本"""
        design_lines = []
        for claim in claims:
            design_lines.append(f"{claim['statement']} {claim['sources']}")
        return "\n".join(design_lines)
    ...
    async def _act(self) -> Message:
        ...
        if self._is_triggered_by(教学优化建议):  # 第一轮优化
            ...
            try:
                ...
                raw_design = await self.actions[0].run(
                    design=materials[0].content,
                    evaluation=assessment_data["assessment"],
                    advice=assessment_data["optimization"],
                    knowledge_base=self.rc.kb  # 显式传递知识库
                )
                # 应用证据链验证与重构
                new_design = await self._validate_and_reconstruct(raw_design)
            except Exception as e:
            ...
            try:
                raw_design = await self.actions[1].run(
        ...
        return Message(
            ...
            cause_by=type(self.rc.todo),
            # 添加知识库元数据
            metadata={"knowledge_base": self.rc.kb.metadata}
        )
...
class 学生智能体(Role):
    ...
    async def _act(self) -> Message:
        ...
        if last_msg and last_msg.cause_by == 教学流程执行:
            ...
            if self.peer_inbox:
                peer_item = self.peer_inbox.pop(0)
                peer_message = peer_item["message"]
                topic = peer_item["topic"]

                response = await 同伴互动().run(
                    student_profile=self.student_profile,
...
class 课堂模拟教师(Role):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.set_actions([教学流程执行, 小组协作调度])
        self._watch([学生画像构建器])
