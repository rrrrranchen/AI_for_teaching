...
def get_chat_block():
    with gr.Blocks(theme=gr.themes.Base(), css=".gradio_container { background-color: #f0f0f0; }") as chat:
        ...
        with gr.Row():     
            ...
            with gr.Column(scale=5):
                ...
                with gr.Accordion(label="模型设置", open=True):
                    # 更新模型选项，添加deepseek-r1
                    model = gr.Dropdown(
                        choices=['qwen-max', 'qwen-plus', 'qwen-turbo', 'deepseek-chat', 'deepseek-coder', 'deepseek-reasoner'],
                        label="选择模型",
                        interactive=True,
                        value="qwen-max",
                        scale=2
        ...
        input_message.submit(
            ...
            inputs=[
                input_message, chatbot, model, temperature, max_tokens, 
                history_round, knowledge_base, similarity_threshold, chunk_cnt
            ],  # 移除了deepseek_api_key参数
